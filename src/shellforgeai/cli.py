...
from shellforgeai.commands import remediation as remediation_commands
...
windows_app = typer.Typer(help="Local read-only Windows V1 doctor and status commands.")
...
def _handle_v2_handoff_compare_ask(question: str) -> bool:
    ...
    console.print("")
    typer.echo(handoff_commands._render_v2_handoff_compare_human(payload), nl=False)
    console.print("")
...
def _handle_v2_handoff_history_ask(question: str) -> bool:
    ...
    console.print("")
    typer.echo(handoff_commands._render_v2_handoff_history_human(payload), nl=False)
    console.print("")
    console.print("Safe commands:")
    console.print("  shellforgeai handoff history")
...
def _handle_v2_handoff_ask(question: str) -> bool:
    ...
    payload = handoff_commands._build_v2_handoff_payload(from_triage=True)
    console.print("Read-only operator handoff (deterministic ask routing):")
    console.print("")
    typer.echo(handoff_commands._render_v2_handoff_human(payload), nl=False)
    if mutation_cues:
        console.print("Refused mutation part of the request.")
        console.print(
...
recipes_commands.register(recipes_app, recipes_preflight_app)
...
receipt_recovery_execute_commands.register(recipes_receipt_app)
...
def safe_actions(
    ...
    if json_out:
        typer.echo(json.dumps(payload))
        return
    typer.echo(recipes_commands._render_recipe_groups_human(payload), nl=False)
    if target:
    ...
    if preflight_ask:
        ...
        if target:
            preflight = build_preflight_packet(
                "docker.disposable_restart", target, scene=recipes_commands._collect_recipe_scene()
            )
            typer.echo(recipes_commands._render_recipe_preflight_human(preflight), nl=False)
            console.print(
...
ops_commands.register(ops_app, ops_report_app)
...
remediation_commands.register(remediation_app, app)
