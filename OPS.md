...
PR191 extracts the governed receipt history/audit/export/compare/rollback-preview handlers (`recipes receipt
history`, `inspect`, `export`, `export-validate`, `compare`, `compare-latest`,
`audit`, `audit-bundle`, `audit-bundle-validate`, `integrity`, `explain`, and
`rollback-preview`) into `commands/receipt_audit.py`, and PR189 extracts the
read-only recipe
...
Receipt history/inspect/compare/audit/integrity/explain/rollback-preview and
audit-bundle/export validation stay read-only; export and audit-bundle stay
bounded ShellForgeAI-owned artifact-only writes. Recipe list/eligibility/
preflight stay read-only and never execute; governed `recipes execute` and
receipt recovery execution remain separately guarded in `cli.py` and unchanged.
