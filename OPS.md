...
After an archive bundle and archive eligibility review exist, operators can generate a non-executable source-action dry-run manifest:
...
For the consolidated archive/source-action review path, use [`docs/ARCHIVE_SOURCE_ACTION_RUNBOOK.md`](docs/ARCHIVE_SOURCE_ACTION_RUNBOOK.md). The runbook documents the existing PR239-PR244 command chain, artifact map, status meanings, and operator decision matrix. It adds no execution command: after the existing confirm-gated copy-only archive bundle creation runway, the source-action dry run, validation, review packet, decision receipt, readiness gate, and status report remain read-only/reporting only. Source action remains unavailable, cleanup remains unavailable, and future source action would require a separate PR/lane with SeedOfEvil as final merge owner.
