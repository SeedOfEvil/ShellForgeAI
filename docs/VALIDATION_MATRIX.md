...
| `src/shellforgeai/core/approved_change_supplemental_context.py` | targeted_runtime | `test_pr314_approved_change_supplemental_context`, `test_pr311_approved_change_construction_policy`, `test_pr310_legacy_proposal_compatibility`, `test_pr309_approved_change_contract` (inert Stage B reviewed-input contract; no CLI, persistence, mutation, or execution surface) |
| `src/shellforgeai/core/approved_change_subject_construction.py` | targeted_runtime | `test_pr315_approved_change_subject_construction`, `test_pr314_approved_change_supplemental_context`, `test_pr311_approved_change_construction_policy`, `test_pr310_legacy_proposal_compatibility`, `test_pr309_approved_change_contract` (inert Stage B in-memory reviewed subject construction; no CLI, persistence, mutation, capability binding, or execution surface) |
| `src/shellforgeai/core/approved_change_artifact_bundle.py` | targeted_runtime | `test_pr316_approved_change_artifact_bundle`, `test_pr315_approved_change_subject_construction`, `test_pr314_approved_change_supplemental_context`, `test_pr311_approved_change_construction_policy`, `test_pr310_legacy_proposal_compatibility`, `test_pr309_approved_change_contract` (inert Stage B in-memory reviewed-change artifact-bundle contract; no filesystem, writer, loader, CLI, persistence, mutation, capability binding, or execution surface) |
| `src/shellforgeai/core/approved_change_approval_workflow.py` | targeted_runtime | `test_pr318_approved_change_approval_workflow`, `test_pr317_approved_change_artifact_persistence`, `test_pr316_approved_change_artifact_bundle`, `test_pr315_approved_change_subject_construction`, `test_pr314_approved_change_supplemental_context`, `test_pr311_approved_change_construction_policy`, `test_pr310_legacy_proposal_compatibility`, `test_pr309_approved_change_contract` (read-only Stage B approval-binding operation: explicit `approve` decision plus exact bundle-identity and subject-hash confirmations over one persisted bundle loaded through the maintained PR317 exact-ID loader; in-memory attestation and contract only, with no filesystem write, approval persistence, capability-support evaluation, preflight, receipt linkage, CLI, natural-language, or execution surface) |
| `src/shellforgeai/core/approved_change_artifact_persistence.py` | full | `test_pr317_approved_change_artifact_persistence`, `test_pr316_approved_change_artifact_bundle`, `test_pr315_approved_change_subject_construction`, `test_pr314_approved_change_supplemental_context`, `test_pr311_approved_change_construction_policy`, `test_pr310_legacy_proposal_compatibility`, `test_pr309_approved_change_contract` (governed Stage B artifact-bundle publication boundary: bounded explicit-confirmed `ARTIFACT_WRITE` beneath the fixed `<data_dir>/approved_change_artifacts` subtree, atomic no-replace publication, bounded cleanup, read-only loader; no CLI, natural-language, approval, capability, receipt, host-configuration, or execution surface) |
| `src/shellforgeai/core/approved_change_approval_artifact.py` | targeted_runtime | `test_pr319_approved_change_approval_artifact_persistence`, `test_pr318_approved_change_approval_workflow`, `test_pr317_approved_change_artifact_persistence`, `test_pr316_approved_change_artifact_bundle`, `test_pr315_approved_change_subject_construction`, `test_pr314_approved_change_supplemental_context`, `test_pr311_approved_change_construction_policy`, `test_pr310_legacy_proposal_compatibility`, `test_pr309_approved_change_contract` (inert Stage B canonical approval-artifact contract: one fixed `approved-change-approval.json` payload built only from one fully successful PR318 result, non-circular approval-artifact identity and `aca_` ID; no filesystem, writer, loader, CLI, natural-language, persistence, mutation, new approval, capability binding, preflight, receipt, or execution surface) |
| `src/shellforgeai/core/approved_change_approval_persistence.py` | full | `test_pr319_approved_change_approval_artifact_persistence`, `test_pr318_approved_change_approval_workflow`, `test_pr317_approved_change_artifact_persistence`, `test_pr316_approved_change_artifact_bundle`, `test_pr315_approved_change_subject_construction`, `test_pr314_approved_change_supplemental_context`, `test_pr311_approved_change_construction_policy`, `test_pr310_legacy_proposal_compatibility`, `test_pr309_approved_change_contract` (governed Stage B approval-artifact publication boundary: bounded explicit-confirmed `ARTIFACT_WRITE` beneath the fixed `<data_dir>/approved_change_approvals` subtree, atomic no-replace publication, bounded cleanup, exact-ID read-only loader revalidating the PR309 binding and the read-only PR317 source bundle; no CLI, natural-language, new approval, mutable approval state, overwrite, revocation, capability, receipt, host-configuration, or execution surface) |
...
| `scripts/windows_runtime_integrity.py` / `scripts/windows_runtime_integrity_acceptance.py` | targeted_runtime | `test_pr304_windows_runtime_integrity` + PR292 runtime-resolution tests |
| `src/shellforgeai/core/windows_runtime_reconcile_execution.py` | **full** | `test_pr313_windows_runtime_reconcile_execute`, `test_pr313_windows_runtime_reconcile_receipt`, `test_pr313_windows_runtime_reconcile_verify`, `test_pr304_windows_runtime_integrity`, `test_pr305_windows_runtime_reconcile`, `test_pr312_v1_cross_cwd_resource_resolution`, `test_pr154_v2_recipe_registry`, `test_pr167_v2_recipe_execute_disposable_restart`, `test_pr174_v2_recipe_receipt_integrity` + `python scripts/run_full_pytest.py` |
| `scripts/windows_runtime_reconcile_execute.py` | **full** | PR313 execute/receipt/verify tests + `test_pr304_*`, `test_pr305_*` + `python scripts/run_full_pytest.py` |
| `scripts/windows_runtime_reconcile_receipt_acceptance.py` | **full** | `test_pr313_windows_runtime_reconcile_receipt`, `test_pr313_windows_runtime_reconcile_execute`, `test_pr174_v2_recipe_receipt_integrity` + `python scripts/run_full_pytest.py` |
| `scripts/windows_runtime_reconcile_verify.py` | **full** | `test_pr313_windows_runtime_reconcile_verify`, `test_pr313_windows_runtime_reconcile_receipt`, `test_pr304_windows_runtime_integrity` + `python scripts/run_full_pytest.py` |
...
## PR313 governed Windows two-file runtime reconciliation execute lane

PR313 introduces the first real bounded local mutation lane for Windows, so it is a Lane C (full) change and the maintained classifier is authoritative. Coverage is split across three focused test files:

- `tests/test_pr313_windows_runtime_reconcile_execute.py` — platform, confirmation, packet, evidence, root, containment, reparse, content, current-state, transaction, backup, atomicity, verification, and compensation behavior.
- `tests/test_pr313_windows_runtime_reconcile_receipt.py` — receipt bundle shape, validator acceptance for every receipt-producing status, tamper rejection, privacy, recipe registry metadata, and preservation of the existing Docker receipt lane.
- `tests/test_pr313_windows_runtime_reconcile_verify.py` — read-only post-change verification acceptance, rejection, privacy, and helper behavior.

PR313 also carries the exact destination-parent contract (contract version 1). `tests/test_pr305_windows_runtime_reconcile.py` gains parent preview/acceptance coverage, and the three PR313 files cover authorized chain creation, safe narrowing, hostile-chain refusal, zero-directory refusals, directory compensation, receipt parent validation, and read-only verification of retained directories. `scripts/windows_runtime_reconcile_preflight.py` and `scripts/windows_runtime_reconcile_acceptance.py` are therefore part of the PR313 Lane C surface.

Regression scope that must stay green alongside PR313: `test_pr304_windows_runtime_integrity`, `test_pr305_windows_runtime_reconcile`, `test_pr312_v1_cross_cwd_resource_resolution`, `test_pr154_v2_recipe_registry`, `test_pr155_v2_recipe_preflight`, `test_pr167_v2_recipe_execute_disposable_restart`, `test_pr174_v2_recipe_receipt_integrity`, `test_pr184_cli_command_surface_golden`, `test_pr205_command_module_import_side_effects`, and `test_pr106_ask_mutation_refusal_routing`.

Linux/Docker validation for this lane is limited to structured `unsupported` proof: the execute and verify helpers return `unsupported` with zero destination mutation, zero backup, zero temporary file, no receipt, and no subprocess/shell/model/network use. Real reconciliation acceptance is a controlled Windows lab phase owned by SeedOfEvil, and the PR312 V1 quick/standard checks from real `C:\Windows\System32` are run separately through the exact installed executable rather than from inside the executor.
