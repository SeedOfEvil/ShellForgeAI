...
This archive preserves dated engineering chronology and may contain period-accurate status language, PR references, and implementation notes that are not the current product classification. For current maturity and platform support, see [Product status](../PRODUCT_STATUS.md).

---

- PR320 (July 28, 2026): added a bounded, deterministic, read-only inventory over persisted approval artifacts. Inventory is discovery, not selection or authorization; it creates no persisted index, performs no repair or mutation, and grants no execution eligibility. The canonical contract remains [Approved Change Approval Inventory](../APPROVED_CHANGE_APPROVAL_INVENTORY.md).
- PR319 (July 27, 2026): added canonical immutable approval-artifact persistence in a fixed ShellForgeAI-owned approval subtree, with atomic no-replace publication and an exact-ID loader. Persisted approval remains non-authorizing and non-executable. The canonical contract remains [Approved Change Approval Artifact Persistence](../APPROVED_CHANGE_APPROVAL_ARTIFACT_PERSISTENCE.md).
- PR318 (July 27, 2026): added explicit, read-only approval binding over one exact persisted reviewed-change bundle, requiring exact bundle-identity and subject-hash confirmation. The approval and contract were in-memory only and provided no authorization, capability support, receipt, persistence, or execution eligibility. The canonical contract remains [Approved Change Approval Workflow](../APPROVED_CHANGE_APPROVAL_WORKFLOW.md).

- PR286 (July 6, 2026): Windows interactive operator parity is improved for latency, CPU/memory/disk/process strongest-signal comparison, Windows next-checks, current-host handoff, and mutation-refusal prompts. Contaminated provider output that acknowledges repo/project/system instructions is suppressed in favor of deterministic Windows-local read-only summaries. Broad natural-language execution remains out of scope; no shell, subprocess, PowerShell, WinRM, cleanup/remediation/rollback/recovery execution, service restart, process termination, model/provider/auth change, secret/auth-cache read, or mutation was added. The Windows ask/interactive latency/slow/status/next-check/handoff paths are deterministic or capture-then-gate before stdout, mutation refusal text is ASCII-safe for Windows consoles, and the transcript helper treats negated no-cleanup/no-command-executed wording as safe refusal evidence while still failing true execution claims.
- PR284 (July 5, 2026): Docker01 ownership-fix readiness packet. Adds `scripts/docker01_ownership_fix_readiness.py`, a read-only local-file helper that prepares an operator-reviewed fix path for the broad recursive Dockerfile ownership layer without applying it. It emits deterministic JSON/Markdown, detects the known `chown -R appuser:appuser /data /home/appuser/.codex /opt/shellforgeai` risk, inspects an existing guarded recipe when present, checks confirmation/apply gates, backup/receipt indicators, and absence of obvious unsafe commands, and can consume a PR282/PR283 build-health JSON report. It does not execute recipes, modify `/srv/compose/shellforgeai/Dockerfile`, build, mutate Docker/Compose, prune/remove, cleanup, remediate, rollback, recover, kill processes, restart services, use Proxmox/QGA, PowerShell/WinRM, shell=True, arbitrary command execution, model/network calls, GitHub approval/merge, or cloud apply/merge/push. Recipe execution remains a separate explicit operator action after SeedOfEvil approval.
- Current: ShellForgeAI includes a narrow read-only platform detection foundation, the first `shellforgeai windows doctor` prototype, and the first `shellforgeai windows status` report for Windows/PowerShell V1 preparation (PR262). `shellforgeai platform doctor` classifies Linux, Windows, Darwin, and unknown platforms with Python standard library metadata only. `shellforgeai windows doctor --json` is local-only and read-only: on Windows it emits strict JSON host basics and safety flags; on Linux/Docker and unsupported platforms it returns structured unsupported output pointing to `shellforgeai platform doctor --json`. The Windows doctor does not execute PowerShell, use WinRM/PSRemoting, mutate the Windows VM, collect services/processes/event logs, run Docker/Compose, install packages, call network/model/Codex providers, read secrets/auth caches, perform remediation/rollback/recovery/cleanup, execute natural-language commands, or use `shell=True`. `shellforgeai windows status --json` is local-only and stdlib-only: on Windows it reports safe host basics, Python runtime, hostname/FQDN, and disk usage without PowerShell, WinRM/PSRemoting, mutation, services/processes/event logs, installs, network/model/Codex calls, or secret/auth-cache reads; on Linux/Docker it returns structured unsupported output and points to `shellforgeai platform doctor --json`. Linux/Docker behavior remains unchanged. Windows Server 2025 VM acceptance should verify `shellforgeai windows status --json` and `shellforgeai windows status` after source validation; `shellforgeai windows doctor --json` remains the regression check. Windows-lane PRs should now preserve saved JSON artifacts and validate them with the local-only `scripts/windows_smoke_acceptance.py` helper and use `scripts/windows_smoke_packet.py` when a deterministic saved-artifact QA packet is needed before adding deeper evidence collection. Future sequence: Windows read-only status report, Windows Server 2025 VM smoke, Windows read-only service/process/event-log evidence in separate PRs, then packaging/install spike.
- PR283 (July 5, 2026): Docker01 build-health Dockerfile discovery. Improves `scripts/docker01_build_health_report.py` so the read-only helper can find the active Docker01 compose Dockerfile from the repo root, `/srv/compose/shellforgeai`, or arbitrary current working directories. The helper checks only a small explicit allowlist, supports `--dockerfile /srv/compose/shellforgeai/Dockerfile`, reports candidates checked plus selected path/source/status, and detects the known broad recursive ownership/chown risk read-only. It does not build, repair, cleanup, prune, restart, kill, mutate Docker/Compose/filesystems/snapshots, perform Proxmox/QGA or PowerShell/WinRM work, execute remediation/rollback/recovery/source actions, call models/network, use `shell=True`, or execute Dockerfile contents.
- PR282 (July 5, 2026): Docker01 build-lane health report. Adds `scripts/docker01_build_health_report.py`, a standalone read-only operator helper that emits deterministic JSON or Markdown (`--json`, `--markdown`, `--out-json`, `--out-markdown`) before Docker01 PR lanes. It was added after PR281 deploy validation was blocked by Docker01 host I/O/BuildKit stalls, not PR281 product code, and helps distinguish infrastructure/build-lane readiness from ShellForgeAI product/test failures. The report covers root/Docker-root/workspace disk usage, build-related process summaries, possible Linux `D`-state/uninterruptible I/O indicators, optional bounded Docker CLI read-only availability checks, and the known broad recursive ownership/chown Dockerfile layer pattern. Readiness is `ok`, `attention`, `blocked`, or `unknown`; successful report generation exits 0 even when readiness is not ok. Helper/tests/docs only: no runtime command surface change, no Docker build, no Docker/Compose mutation, no prune/remove, no process kill, no service restart, no filesystem cleanup/delete/repair, no snapshot mutation, no package install, no pytest execution by the helper, no Proxmox/QGA, no PowerShell/WinRM, no model/network/secret/auth-cache behavior, no remediation/rollback/recovery, no natural-language execution, no `shell=True`, no arbitrary command execution, no cloud apply/merge/push, and no GitHub approval/merge.
...
- PR228 (June 21, 2026): Model Doctor live-probe receipt history and compare. `shellforgeai model receipt history --root /tmp [--json]` discovers known receipt-shaped directories under a bounded root, validates them with required-file, JSON, manifest, checksum, secret-marker, and safety checks, and reports valid, invalid, ignored, and latest-valid receipt metadata. `shellforgeai model receipt compare <old_receipt_dir> <new_receipt_dir> [--json]` validates both receipts and reports deterministic probe-status, auth-readiness, latency, timeout, provider, and model drift. Both commands are read-only evidence surfaces only: no live probe, model/Codex/network call, cleanup/prune/delete, artifact repair/move, Docker/Compose mutation, restart, remediation, rollback, recovery, natural-language execution, or product runtime mutation behavior. Default `model doctor` remains no-call; explicit live probe remains opt-in and bounded. SeedOfEvil remains final merge owner.

- PR229 (June 22, 2026): Docker01 QA bundle model receipt evidence integration. The Docker01 operator QA bundle now surfaces existing Model Doctor receipt evidence in the reviewer handoff by default, so a reviewer can see whether model receipt history exists, whether the latest receipt validates, the latest probe status, and whether model/auth readiness was recently verified by an explicit previous live probe — without the QA bundle itself running a live probe or calling the model. The bundle runs only the read-only `shellforgeai model receipt history --root /tmp --json` reader and records a `model_receipts` block in `qa-results.json`, a `Model receipt evidence` section in `qa-summary.md`, the command in `commands-run.json`, and bounded `raw/model-receipt-history.json` / `raw/model-receipt-evidence.json` captures. The bundle's own `model_receipts.safety` always reports `model_called=false`/`live_probe_performed=false`; a historical receipt's `model_called=true` is accepted as evidence of an earlier explicit probe (PR226). Empty/unavailable history is non-blocking (`empty`/`not_available` with a warning), while a secret marker or historical safety drift is surfaced as a blocking safety failure. No live probe, model/Codex/network call, package install, cleanup/prune/delete, artifact repair/move, Docker/Compose mutation, restart, remediation, rollback, recovery, natural-language execution, or product runtime mutation was added; the PR196 model-module static guard is preserved. SeedOfEvil remains final merge owner.


- PR237 (June 25, 2026): Governed artifact archive bundle validator. `scripts/docker01_artifact_archive_plan.py` gains read-only `--validate-archive-bundle <archive_bundle_dir>` support with optional `--plan-dir <plan_dir>`, optional `--dry-run-receipt <receipt_dir>`, and optional `--out <validation_dir>`. The validator checks PR236 bundle required files, JSON parseability, archive manifest, payload checksums, copied payload file presence, unsafe payload paths/symlinks, source-preservation metadata, receipt safety flags, and optional plan/dry-run cross-checks for plan id plus candidate counts/classes/bytes/source paths. `--out` writes validation artifacts only (`artifact-archive-bundle-validation.json`, summary, manifest, checksums). The validator itself reports `read_only=true`, `mutation_performed=false`, `archive_created=false`, and `source_copied=false`; it never creates archives, copies/moves/modifies/deletes sources, authorizes cleanup, runs cleanup/prune/delete/restart/remediation/rollback/recovery, mutates Docker/Compose, invokes validation/pytest/QA from the helper, uses natural-language execution or `shell=True`, or performs network/model/Codex/package/GitHub/cloud actions. Source deletion/move remains out of scope and would require a separate PR/lane and confirmation. SeedOfEvil remains final merge owner.

- PR235 (June 23, 2026): Governed artifact archive execution-readiness gate. `scripts/docker01_artifact_archive_plan.py` gains read-only `--execution-readiness <plan_dir> --dry-run-receipt <receipt_dir>` support, optionally with `--receipt-validation <validation_dir>`, to validate the PR231/PR232 plan and PR233/PR234 dry-run receipt evidence chain before any future archive mutation lane exists. The gate cross-checks exact plan id, candidate paths/counts/classes/bytes, exclusions, confirmation phrase, future source delete/move defaults, execution-unavailable flags, and mutation safety flags, then reports `ready_for_execution_review|not_ready|failed|partial` as strict JSON or concise human output. `--out <dir>` writes readiness report artifacts only (`artifact-archive-execution-readiness.json`, summary, future checklist, safety notes, manifest, checksums). It never creates an archive, copies/moves/modifies/deletes source artifacts, modifies source plan/receipt directories, runs cleanup/prune/delete/restart/remediation/rollback/recovery, executes Docker/Compose mutation, invokes validation/pytest/QA from the helper, uses natural-language execution or `shell=True`, performs network/model/Codex/package/GitHub/cloud actions, or adds product runtime mutation behavior. Future execution remains unavailable and separate, requiring exact plan id and `CONFIRM_SHELLFORGEAI_ARTIFACT_ARCHIVE`; SeedOfEvil remains final merge owner.

- PR234 (June 23, 2026): Governed artifact archive dry-run receipt validator. `scripts/docker01_artifact_archive_plan.py` gains read-only `--validate-dry-run-receipt <receipt_dir>` support with optional `--plan-dir <plan_dir>` cross-check and optional `--out <validation_dir>` validator artifacts. The validator checks receipt required files, JSON parseability, manifest, checksums, safety flags, candidate scope, future confirmation/source-preservation contract, explicit execution unavailability, and (when supplied) the original plan id plus candidate counts/classes/bytes and contract consistency. Standalone validation records `plan_cross_check_status=not_requested`. It never creates an archive, copies/moves/modifies/deletes source artifacts, modifies source receipt/plan directories, runs cleanup/prune/delete/restart/remediation/rollback/recovery, executes Docker/Compose mutation, invokes validation/pytest/QA from the helper, uses natural-language execution or `shell=True`, performs network/model/Codex/package/GitHub/cloud actions, or adds product runtime mutation behavior. Future execution remains unavailable and separate; SeedOfEvil remains final merge owner.

- PR233 (June 22, 2026): Governed artifact archive dry-run receipt. `scripts/docker01_artifact_archive_plan.py` gains read-only `--dry-run-receipt <plan_dir> --plan-id <plan_id>` support that first reuses PR232 validation, requires an exact plan id, fails clearly on missing/mismatched ids, and emits strict JSON or concise human Markdown summarizing a future archive preview, candidate counts/classes/bytes, explicit exclusions, the `CONFIRM_SHELLFORGEAI_ARTIFACT_ARCHIVE` phrase, receipt/manifest/checksum/rollback requirements, and source-preservation requirements. `--out <dir>` writes dry-run receipt metadata only (`artifact-archive-dry-run-receipt.json`, `artifact-archive-dry-run-summary.md`, `candidate-manifest.json`, `excluded-candidates.json`, `future-execution-checklist.md`, `safety-notes.md`, `manifest.json`, `checksums.json`). Execution remains unavailable (`execution_available=false`, `dry_run_only=true`): no archive is created, no source is copied/moved/modified/deleted, no source plan directory is modified, no cleanup/prune/delete/restart/remediation/rollback/recovery or Docker/Compose mutation runs, no helper-triggered validation/pytest/QA, no natural-language execution, no `shell=True`, no network/model/Codex/package install/GitHub/cloud action, and no product runtime mutation behavior is added. Future archive execution remains a separate PR/lane gated by exact plan id and confirmation phrase. SeedOfEvil remains final merge owner.

- PR231 (June 22, 2026): Governed ShellForgeAI artifact archive plan contract. New read-only helper `scripts/docker01_artifact_archive_plan.py` discovers bounded ShellForgeAI-owned historical evidence artifacts under a supplied root (default `/tmp`), classifies known candidate classes (QA bundles, validation, merge/v2 readiness, hygiene, model receipts, receipt validation, and storage-health artifacts), estimates counts/bytes, emits a deterministic `plan_id`, and documents explicit exclusions plus future confirmation, receipt, manifest, checksum, rollback-instruction, and operator-review requirements. `--json` emits strict JSON; human output is concise and pasteable; `--out <dir>` writes plan metadata files only (`artifact-archive-plan.json`, `artifact-archive-plan-summary.md`, `candidate-manifest.json`, `excluded-candidates.json`, `safety-notes.md`, `manifest.json`, `checksums.json`). Execution is explicitly unavailable (`execution_available=false`): no archive is created, no candidate contents are copied, no source is moved/modified/deleted, no cleanup/prune/delete/restart/remediation/rollback/recovery/validation/QA execution runs, no Docker/Compose mutation, no natural-language execution, no `shell=True`, no network/model/Codex/package install/GitHub/cloud action, and no product runtime mutation behavior is added. Any future execution lane would be separate and require exact plan id, exact `CONFIRM_SHELLFORGEAI_ARTIFACT_ARCHIVE` phrase, bounded candidate classes, validated manifest, archive/receipt targets, dry-run preview, and operator review. SeedOfEvil remains final merge owner.

- PR230 (June 22, 2026): Docker01 storage health evidence report. New read-only helper `scripts/docker01_storage_health_report.py` collects bounded evidence about Docker01 storage health after PR229 observed a slow Docker build user/group/chown layer and pre-existing EXT4 journal/inode warning/error-count messages on `dm-10`. `python scripts/docker01_storage_health_report.py --json` emits strict JSON (`schema_version=1`, `mode=docker01_storage_health_report`) with root filesystem capacity, mounted filesystems / device mapping, disk-pressure level, Docker data-path pressure when safely readable, and bounded/sanitized kernel storage warning lines scored against EXT4, dm/device-mapper, and I/O/journal/inode patterns; `--out <dir>` additionally writes `storage-health-report.json`, `storage-health-summary.md`, `commands-run.json`, `manifest.json`, and `checksums.json` (SHA256 + sizes), with raw logs not copied in full. `status` is `ok`/`warning`/`partial`/`failed`: `partial` when `dmesg`/`journalctl` access is denied but core disk usage is collected, `failed` only when core disk usage cannot be collected. It uses a fixed read-only command allowlist (`df -P -B1`, `findmnt --json`, `dmesg --level=err,warn --ctime`, `journalctl -k -p warning..alert --no-pager -n <bounded>`) plus `shutil.disk_usage` and `/proc/mounts`, always `shell=False`, with optional `--max-dmesg-lines`/`--max-warning-lines` bounds and a `/proc/mounts` fallback when `findmnt` is absent. This PR is evidence-only: no filesystem repair, `fsck`/`e2fsck`/`xfs_repair`, mount/remount/umount, Docker prune/image/volume/container removal, file deletion, stale-artifact repair/move/delete, container restart, Docker/Compose mutation, remediation/rollback/recovery execution, natural-language execution, `shell=True`, arbitrary command execution, package install, network/model/Codex call, GitHub post/approve/merge, or cloud apply/merge/push; no `--execute`/`--apply`/`--cleanup`/`--delete`/`--prune`/`--repair`/`--fsck`/`--restart`/`--fix` switches were added, and no product CLI/runtime mutation behavior changed. New `tests/test_pr230_docker01_storage_health_report.py` covers JSON/human output, warning-pattern detection and bounding, output files/manifest/checksums, partial-on-denied-dmesg, and the all-false safety block. Docs: `OPS.md`, `README.md`, `docs/VALIDATION_LANES.md`, `docs/VALIDATION_MATRIX.md`, `docs/roadmap.md`. SeedOfEvil remains final merge owner.

- PR232 (June 22, 2026): Artifact archive plan validator. `scripts/docker01_artifact_archive_plan.py` gains a read-only `--validate <plan_dir>` mode that validates an existing PR231 plan directory without creating an archive or touching any source. It confirms the required plan files exist and parse, the manifest and checksums match the current plan output files (SHA256 + size), the `plan_id` is present and well-formed, `read_only=true`/`mutation_performed=false`/`execution_available=false`, the candidate manifest is bounded and limited to known ShellForgeAI evidence patterns (out-of-scope paths and symlink candidates are rejected and never followed), the `CONFIRM_SHELLFORGEAI_ARTIFACT_ARCHIVE` confirmation phrase is present, the future contract keeps execution unavailable and source deletion out of scope, and every safety mutation flag is false. `--json` emits strict JSON (`mode=docker01_artifact_archive_plan_validation`, `status=passed|failed|partial`, `future_execution_available=false`, `future_execution_eligible_for_review`, and an all-false safety block); human output is concise and pasteable. `--out <dir>` writes validator artifacts only (`artifact-archive-plan-validation.json`, `artifact-archive-plan-validation-summary.md`, `manifest.json`, `checksums.json`) and never modifies the source plan directory. Validation is read-only: it creates no archive, copies/moves/deletes no source, runs no cleanup/prune/restart/remediation/rollback/recovery, performs no Docker/Compose mutation, no natural-language execution, no `shell=True`, no GitHub/cloud apply/merge/push, and authorizes no execution — future execution remains a separate lane gated on the exact plan id and confirmation phrase. New `tests/test_pr232_docker01_artifact_archive_plan_validation.py` covers passed/empty/failure paths (missing files, invalid JSON, checksum/manifest mismatch, mutation/execution/safety flags, missing confirmation phrase, unsafe and symlink candidates), `--out` artifacts, source-immutability, and the no-mutation static guard. Docs: `OPS.md`, `README.md`, `docs/roadmap.md`. SeedOfEvil remains final merge owner.
...
- Windows interactive slow-system route is platform-aware and read-only (PR279): interactive slow-system/performance diagnostics detect the platform through the existing read-only platform detection helpers before any collector runs. On Windows the route no longer executes Linux-only collectors (`uptime`, `df`, `ip`, `ss`, `ps`, `systemctl`, `/proc` reads, `/etc/resolv.conf` reads); it records them as structured `linux_only_collector_skipped` evidence, renders missing metrics as explicit `windows_metric_unavailable` markers instead of `loadavg=None` or fake `0.0GiB/0.0GiB` memory, reuses only the existing stdlib-only `windows status`/`windows disks` read-only payloads, and points at safe next commands such as `shellforgeai windows status --json` and `shellforgeai windows processes --json --limit 10`. The route stays read-only and non-mutating, requires no model synthesis (the deterministic Windows summary renders when the model is unavailable), adds no new Windows collection surface, executes no PowerShell, and uses no WinRM/remoting. PR278 JSON-null collector parsing behavior is preserved and Linux/Docker performance diagnostics are unchanged. Saved-transcript acceptance coverage is available through `scripts/windows_interactive_acceptance.py`, which validates the Windows-aware performance transcript and unsafe mutation-refusal transcript without launching ShellForgeAI. Deeper Windows performance metrics (CPU/memory counters, per-process usage, event-log signals) remain future work in separate PRs.
- Windows evidence bundle opt-in processes component: `shellforgeai windows evidence --include-processes [--processes-limit N]` (PR276) embeds the existing PR274 read-only processes payload in the evidence bundle as an explicit, bounded, opt-in component with a conservative default limit of 25 (validated range 1-200, matching the standalone processes bounds; `--processes-limit` is valid only with `--include-processes`). The default bundle remains doctor/status-only and PR264-compatible, and the PR269 services and PR272 disks opt-in behaviors are unchanged (services, disks, and processes can be combined for a component count of 5). It reuses the existing read-only PR274 processes payload builder and adds no new Windows collection surface: no command-line collection, no environment collection, no process memory reads, no handles/modules/owners/users/tokens inspection, no network connection mapping, no process termination/control, no PowerShell execution, no WinRM/remoting, and no cleanup/remediation/rollback/recovery. The saved-artifact validator and packet helper understand evidence bundles with embedded processes (bounded fields, `embedded_processes` summary block, per-item field allowlist, cross-check against a standalone processes artifact when both are provided), and standalone `windows-processes.json` support from PR275 remains valid. Event logs, firewall evidence, and Windows Update evidence remain future separate PRs.
- Windows processes saved-artifact validation and packet support: `scripts/windows_smoke_acceptance.py` and `scripts/windows_smoke_packet.py` accept an optional `--processes-json` path (PR275) to validate and report saved PR274 `windows_processes` artifacts (schema, `windows_processes` mode, `ctypes_toolhelp32_snapshot` method, bounded limit/truncated/count fields, the per-item field allowlist of `pid`/`parent_pid`/`name`/`thread_count`, the `not_collected_in_pr274` notes, processes safety flags, hash/size/mode/status, and a safe processes summary in the packet JSON/Markdown). PR275 validates saved artifacts only: it does not run ShellForgeAI product commands, does not collect new process data, does not add processes to the evidence bundle, does not execute PowerShell, does not use WinRM/remoting, and does not mutate the Windows VM. It validates that process artifacts do not include command lines, environments, memory, handles, modules, owners/users, or network connections. The helpers read saved local files only (UTF-8, UTF-8 BOM, UTF-16 with BOM, and Windows PowerShell 5.1 UTF-16LE/BOM) and add no subprocess, network/model calls, or mutation. Evidence-bundle integration for processes landed in PR276.
- Windows disks safety-flag normalization (PR273): the standalone `shellforgeai windows disks --json` payload and the embedded disks component in `shellforgeai windows evidence --json --include-disks` now both explicitly report `directory_scan_performed=false`, `file_scan_performed=false`, and `disk_mutation_performed=false` in their safety blocks, matching the top-level PR272 evidence safety block and closing the PR272 reviewer note about the embedded disks payload missing the newer disk-specific safety keys. `scripts/windows_smoke_acceptance.py` requires the explicit disk safety flags for PR273+ disks artifacts (standalone and embedded; legacy artifacts missing `disk_mutation_performed` fail strict validation with a clear check name), and `scripts/windows_smoke_packet.py` reports the three flags in the disks artifact summary. This is safety-schema consistency only: no new disk collection, no directory/file scans, no disk mutation, no PowerShell/WinRM/remoting, and no change to default evidence, opt-in, limit, or services behavior.
- Windows evidence bundle opt-in disks component: `shellforgeai windows evidence --include-disks [--disks-limit N]` (PR272) embeds the existing PR270 read-only disks payload in the evidence bundle as an explicit, bounded, opt-in component with the same safe default limit of 32 (validated range 1-64). The default bundle remains doctor/status-only and PR264-compatible, and the PR269 services opt-in behavior is unchanged (services and disks can be combined). It reuses the existing read-only disks payload builder and adds no new Windows collection surface: no directory or file scanning, no disk mutation (no mount/unmount/format/repair), no PowerShell execution, no WinRM/PSRemoting, no registry reads/writes, and no cleanup/remediation/rollback/recovery. The saved-artifact validator and packet helper understand evidence bundles with embedded disks (bounded fields, `embedded_disks` summary block, cross-check against a standalone disks artifact when both are provided), and standalone `windows-disks.json` support from PR271 remains valid. Process inventory, event logs, firewall evidence, and Windows Update evidence remain future separate PRs.
- Windows disks saved-artifact validation and packet support: `scripts/windows_smoke_acceptance.py` and `scripts/windows_smoke_packet.py` accept an optional `--disks-json` path (PR271) to validate and report saved PR270 `windows_disks` artifacts (schema, `windows_disks` mode, bounded limit/truncated/root-count fields, disks safety flags, sanitized unavailable-root handling, hash/size/mode/status, and a safe disk summary in the packet JSON/Markdown). Unavailable roots are accepted only when sanitized as safe disk usage failures such as `disk_usage_failed`; tracebacks or raw exception detail fail validation. The helpers read saved local files only (UTF-8, UTF-8 BOM, UTF-16 with BOM, and Windows PowerShell 5.1 UTF-16LE/BOM) and add no new Windows collection, no product command changes, no PowerShell, no WinRM, no subprocess, no network/model calls, and no mutation. Opt-in evidence bundle integration for disks landed in PR272. Deeper disk inspection, disk cleanup, disk repair, mount/format, services mutation, process inventory, event logs, firewall evidence, and Windows Update evidence remain future separate PRs or explicitly out of scope.
- Windows local disks read-only preview: `shellforgeai windows disks [--json] [--limit N]` (PR270) is the next standalone Windows read-only evidence slice: a local disk/root usage preview. On Windows it discovers local drive roots with `os.listdrives` when available (feature-detected; otherwise it falls back safely to the current drive root only) and collects per-root total/used/free bytes via `shutil.disk_usage`, using the Python standard library only, with a bounded deterministic `--limit` (default 32, range 1-64) and sanitized per-root `unavailable` entries instead of tracebacks. It does not scan directories or files, does not read user files, does not read secrets or auth caches, does not execute PowerShell, does not use WinRM/PSRemoting, does not use subprocess, does not collect drive labels, volume serials, BitLocker status, SMART/health status, or file/directory inventory, and does not mutate the Windows VM. Linux/Docker and unsupported platforms return structured unsupported output pointing to `shellforgeai platform doctor --json`. Saved-artifact validator/packet support for disks is complete since PR271, and since PR272 the disks preview is also available inside `shellforgeai windows evidence` as an explicit, bounded, opt-in component via `--include-disks`.
- Windows evidence bundle opt-in services component: `shellforgeai windows evidence --include-services [--services-limit N]` (PR269) embeds the existing PR267 read-only services collector output in the evidence bundle as an explicit, bounded, opt-in component with a conservative default limit of 25 (validated range 1-500). The default bundle remains doctor/status-only and PR264-compatible. This was the next small Windows evidence integration after the standalone services surface was proven safe; it adds no new Windows collection surface, no PowerShell execution, no WinRM/PSRemoting, no subprocess, no service control/restart/config mutation, no registry or execution-policy changes, and no mutation. The saved-artifact validator and packet helper accept evidence bundles with embedded services, apply the same key safety expectations as the standalone services artifact, and cross-check embedded vs standalone services artifacts when both are provided. Process inventory, event logs, firewall evidence, and Windows Update evidence remain future separate PRs.
- Windows services saved-artifact validation and packet support: `scripts/windows_smoke_acceptance.py` and `scripts/windows_smoke_packet.py` accept an optional `--services-json` path to validate and report saved PR267 `windows_services` artifacts (schema, summary counts, truncation-limit consistency, safety flags, hash/size/mode/status, and service count summary in the packet JSON/Markdown). Services saved-artifact validation is the QA gate before deeper Windows evidence slices. The helpers read saved local files only and add no new Windows collection, no product command changes, no PowerShell, no WinRM, no subprocess, no network/model calls, and no mutation. Service deep detail, process inventory, event logs, firewall evidence, and Windows Update evidence remain future separate PRs.
- Windows local services read-only preview: `shellforgeai windows services [--json]` (PR267) is the first narrow deeper Windows evidence slice: a standalone local read-only service state summary. On Windows it collects service names, display names, and current states only, via read-only `ctypes` Service Control Manager enumeration, and summarizes counts by state with a bounded collection limit. It does not execute PowerShell, does not use WinRM/PSRemoting, does not use subprocess, does not start/stop/restart/control/configure services, does not read service binary paths/accounts/config or the registry, and does not mutate the Windows VM. Linux/Docker and unsupported platforms return structured unsupported output pointing to `shellforgeai platform doctor --json`. Since PR269 the services preview is also available inside `shellforgeai windows evidence` as an explicit, bounded, opt-in component via `--include-services`. Service deep detail, process inventory, event logs, firewall evidence, and Windows Update evidence remain future separate PRs.
- Windows read-only evidence bundle preview: `shellforgeai windows evidence [--json]` is a local bundle/preview command that reuses the existing Windows doctor and status payloads. It adds no new evidence collection, does not execute PowerShell, does not use WinRM/PSRemoting, does not mutate Windows hosts or VMs, and keeps service inventory, process inventory, event logs, firewall evidence, and Windows Update evidence as separate future PRs.

- Docker01 ownership candidate verifier: the repository now contains `ops/docker/Dockerfile.docker01.ownership-candidate` plus `scripts/docker01_build_path_candidate_verify.py` for static review of safer Docker01 build-path ownership. The candidate is not the active production Dockerfile; the verifier does not edit `/srv/compose/shellforgeai/Dockerfile`, edit Compose, run Docker/Compose/build, run `chown`/`chmod`/`chgrp`, install packages, clean up, prune, restart, remediate, roll back, or recover. Any actual Dockerfile/build remediation remains a separate PR or operator-reviewed change.
- Docker01 ownership handoff packet: `scripts/docker01_build_path_ownership_handoff_packet.py` creates a read-only operator handoff packet from the explicit external Docker01 Dockerfile and repository-owned ownership candidate. It emits evidence, source/candidate SHA256 values, source-vs-candidate diff, operator checklist, future-change preflight, and rollback-planning notes only; it does not edit `/srv/compose/shellforgeai/Dockerfile`, edit Compose, run Docker/Compose/build, run `chown`/`chmod`/`chgrp`, install packages, clean up, prune, restart, remediate, roll back, recover, or authorize production changes. Any actual Dockerfile/build remediation remains a separate PR or operator-reviewed change.
...
### Docker01 QA bundle model receipt evidence integration

Docker01 PR QA bundles now also connect the Model Doctor receipt evidence trail (PR226 receipts, PR227 validator, PR228 history) to the operator handoff. By default the bundle runs the read-only `shellforgeai model receipt history --root /tmp --json` reader and records whether receipt history exists, whether the latest receipt validates, the latest probe status, the latest auth readiness, valid/invalid counts, warnings, and the operator's safe next command, plus bounded raw captures. Operators can opt out with `--skip-model-receipts`.
...
- Current Docker01 artifact archive runway: the source-action operator status report consumes PR243 readiness evidence, optionally cross-checks PR239–PR242 and archive runway artifacts with an exact plan id, and emits read-only operator-facing status artifacts only. `ready_for_operator_review` means inspectable for a future separate PR/lane, not approval or execution; it does not authorize cleanup/source action, create a source-action command, create archives, copy/move/modify/delete sources, mutate Docker/Compose, or run cleanup/prune/delete/restart/remediation/rollback/recovery. SeedOfEvil remains final merge owner.
...
### Windows saved evidence packet helper

The saved evidence packet helper converts existing Windows smoke evidence/status/doctor JSON artifacts into deterministic JSON or Markdown handoff reports. It reuses the saved-artifact validator, records SHA256 hashes and byte sizes, and adds no Windows collection. Services, processes, event logs, firewall evidence, and Windows Update remain future separate PRs.

### Windows local services read-only preview

PR267 adds `shellforgeai windows services`, a standalone local read-only Windows service state summary preview: service names, display names, and current states only, with deterministic state counts and a bounded collection limit, collected through read-only `ctypes` Service Control Manager enumeration. It adds no PowerShell execution, no WinRM/PSRemoting, no subprocess, no service start/stop/restart/control/configuration, no service binary path/account/config collection, no registry reads/writes, and no Windows mutation. It is not yet part of `shellforgeai windows evidence`; bundle integration may follow only after the standalone surface is proven safe. Services/process deep detail, event logs, firewall evidence, and Windows Update remain future separate PRs.

### Windows services saved-artifact validation

PR268 teaches the saved-artifact acceptance validator and packet helper to validate and report saved `windows-services.json` artifacts through an optional `--services-json` path. This is the QA gate before deeper Windows evidence slices: it reads saved local files only (UTF-8, UTF-8 BOM, UTF-16 with BOM, and Windows PowerShell 5.1 UTF-16LE/BOM), adds no Windows collection and no product command changes, and keeps process inventory, event logs, firewall evidence, and Windows Update evidence as future separate PRs.

### Windows evidence bundle opt-in services component

PR269 is the next small Windows evidence integration after the standalone services surface was proven safe: `shellforgeai windows evidence --include-services [--services-limit N]` embeds the existing PR267 read-only services collector output in the evidence bundle as an explicit, bounded, opt-in component (default limit 25, validated range 1-500). The default bundle remains doctor/status-only. It reuses the existing services collector, adds no new Windows collection surface, no PowerShell execution, no WinRM/PSRemoting, no service control/restart/config mutation, no registry changes, and no mutation. The saved-artifact validator and packet helper accept and summarize evidence bundles with embedded services. Process inventory, event logs, firewall evidence, and Windows Update evidence remain future separate PRs.

### Windows local disks read-only preview

PR270 adds `shellforgeai windows disks`, a standalone local read-only Windows disk/root usage preview: local drive roots discovered with feature-detected `os.listdrives` (with a safe current-drive-root-only fallback) and per-root total/used/free bytes from `shutil.disk_usage`, stdlib-only, bounded by a deterministic `--limit` (default 32, range 1-64). It adds no directory or file scanning, no user-file reads, no secret/auth-cache reads, no PowerShell execution, no WinRM/PSRemoting, no subprocess, no drive label/volume serial/BitLocker/SMART/file-inventory collection, no registry reads/writes, and no Windows mutation. Disk evidence is not yet part of `shellforgeai windows evidence`; the sequence is standalone disks preview first, then saved-artifact validator/packet support for disks (PR271), then opt-in evidence bundle integration for disks only if PR270 is stable.

### Windows disks saved-artifact validation

PR271 teaches the saved-artifact acceptance validator and packet helper to validate and report saved `windows-disks.json` artifacts through an optional `--disks-json` path, accepting unavailable roots only when sanitized as safe disk usage failures (for example `disk_usage_failed`). Disks saved-artifact validation and packet support are complete. It reads saved local files only (UTF-8, UTF-8 BOM, UTF-16 with BOM, and Windows PowerShell 5.1 UTF-16LE/BOM), adds no Windows collection and no product command changes, and keeps deeper disk inspection, disk cleanup, disk repair, mount/format, services mutation, process inventory, event logs, firewall evidence, and Windows Update evidence as future separate PRs or explicitly out of scope.

### Windows evidence bundle opt-in disks component

PR272 is the next small Windows evidence integration after the standalone disks surface was proven safe: `shellforgeai windows evidence --include-disks [--disks-limit N]` embeds the existing PR270 read-only disks payload in the evidence bundle as an explicit, bounded, opt-in component (default limit 32, validated range 1-64). The default bundle remains doctor/status-only, and the PR269 services opt-in behavior is unchanged. It reuses the existing disks payload builder, adds no new Windows collection surface, no directory or file scanning, no disk mutation, no PowerShell execution, no WinRM/PSRemoting, no registry changes, and no cleanup/remediation/rollback/recovery. The saved-artifact validator and packet helper accept and summarize evidence bundles with embedded disks; standalone `windows-disks.json` support from PR271 remains valid. Process inventory, event logs, firewall evidence, and Windows Update evidence remain future separate PRs.


## Windows local processes preview

`shellforgeai windows processes [--json] [--limit N]` is available as a standalone local read-only bounded Windows process preview. It collects only PID, parent PID, process image basename/name, and thread count, with a default limit of 50 and a maximum of 200. It does not execute PowerShell, use WinRM/remoting, terminate/control/suspend processes, read command lines, read environments, inspect memory, handles, modules, owners/tokens, or map network connections. Linux/Docker and unsupported hosts return structured unsupported output without probing Linux/Docker processes. Saved-artifact validator/packet support for `windows-processes.json` is complete since PR275; since PR276 the processes preview is also available inside `shellforgeai windows evidence` as an explicit, bounded, opt-in component via `--include-processes`, and services and disks behavior remains unchanged.

## Windows processes saved-artifact validation

PR275 teaches the saved-artifact acceptance validator and packet helper to validate and report saved `windows-processes.json` artifacts through an optional `--processes-json` path. It validates saved artifacts only: it does not run ShellForgeAI product commands, does not collect new process data, does not add processes to the evidence bundle, does not execute PowerShell, does not use WinRM/remoting, and does not mutate the Windows VM. It validates that process artifacts carry only PID, parent PID, image basename/name, and thread count, and never command lines, environments, memory, handles, modules, owners/users, or network connections. It reads saved local files only (UTF-8, UTF-8 BOM, UTF-16 with BOM, and Windows PowerShell 5.1 UTF-16LE/BOM). Evidence-bundle integration for processes landed in PR276; process termination/control, process memory/command-line/environment reads, handles/modules/owner inspection, network connection mapping, and service/process correlation remain out of scope.

## Windows evidence bundle opt-in processes component

PR276 is the next small Windows evidence integration after the standalone processes surface was proven safe: `shellforgeai windows evidence --include-processes [--processes-limit N]` embeds the existing PR274 read-only processes payload in the evidence bundle as an explicit, bounded, opt-in component (default limit 25, validated range 1-200; `--processes-limit` is valid only with `--include-processes`). The default bundle remains doctor/status-only, and the PR269 services and PR272 disks opt-in behaviors are unchanged; services, disks, and processes can be combined for a component count of 5. It reuses the existing PR274 processes payload builder, adds no new Windows collection surface, no command-line/environment/memory/handles/modules/owner collection, no network connection mapping, no process termination/control, no PowerShell execution, no WinRM/remoting, and no cleanup/remediation/rollback/recovery. The saved-artifact validator and packet helper accept and summarize evidence bundles with embedded processes; standalone `windows-processes.json` support from PR275 remains valid. Event logs, firewall evidence, and Windows Update evidence remain future separate PRs.

- Windows smoke packet interactive transcript integration (PR281): `scripts/windows_smoke_packet.py` now covers saved JSON artifacts plus optional saved interactive slow/mutation-refusal transcripts through `--slow-transcript` and `--mutation-transcript`. The helper reuses the PR280 transcript acceptance checks, records transcript path/SHA256/size/accepted state, emits an interactive JSON summary and an "Interactive transcript summary" Markdown section, and fails the packet on failed transcript acceptance. This is QA/handoff only: saved local files are read, no interactive mode is launched, no product behavior changes, no PowerShell/WinRM/QGA/Proxmox/subprocess/network/model calls, and no mutation. Deeper Windows performance metrics remain future work.
