...
Every newly collected packet has an exact `observation` mapping: `capture_started_at_utc`, `capture_completed_at_utc`, `capture_duration_ms`, and `clock_source`. UTC values use fixed-microsecond RFC3339 form (`YYYY-MM-DDTHH:MM:SS.ffffffZ`) from the local system wall clock; the exact clock-source literal is `local_system_utc_plus_monotonic`. The collector reads UTC and the monotonic clock immediately before bounded host observation, then reads the monotonic clock and UTC immediately after it. Integer milliseconds come only from the monotonic difference. The maintained 600,000 ms maximum is corruption protection for packet structure, not a freshness TTL.

Validation requires the exact four-field shape when `observation` is present, strict canonical UTC strings, completion at or after start, a non-boolean integer duration from 0 through 600,000, and the maintained clock-source literal. Pre-capture schema-1 packets remain accepted as explicit legacy observations with chronology unavailable and invalid; no filesystem time, current time, approval time, or other surrogate is invented. Unsupported-host packets still carry capture chronology because platform detection is itself inside the window.

These clock readings are observational only. Wall time is unauthenticated local system UTC; no NTP, domain-controller, network synchronization check, or correction occurs. Monotonic time proves only local elapsed process duration. Chronology is not freshness, state may change immediately after completion, and neither chronology nor exact identity grants authorization, preflight, or execution authority. Clock reads perform no host mutation.
