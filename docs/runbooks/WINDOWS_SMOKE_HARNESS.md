...
distinct BOM-less UTF-8 representation. The normalizer accepts strict UTF-8,
...
6. Capture the normal Windows smoke artifact set:
   ...
   `windows-processes.json`, and `windows-memory.json`.
...
The helper reads saved transcript files only. It does not launch interactive mode, run ShellForgeAI commands, execute PowerShell, use WinRM, contact QGA/Proxmox, call models, or perform mutation. It accepts UTF-8, UTF-8 with BOM, UTF-16 with BOM, and Windows PowerShell 5.1 UTF-16LE/BOM transcript files. The checks require no traceback, no PR278 JSON-null crash (`ValueError: malformed node or string` / `Name(id='null')`), no Linux fake metrics (`loadavg=None` or `0.0GiB/0.0GiB`), Windows-aware skipped/unavailable metric wording, safe follow-up guidance, and refusal of unsafe cleanup/restart requests. Valid real PR279 slow-transcript markers include `Collected ... read-only evidence item(s).`, `Visibility: windows-local-read-only`, and Linux-only collector skip messages. Safe negative refusal wording such as `No command was executed.` is treated as non-execution, while true execution indicators such as cleanup/remediation/rollback/recovery execution, Docker restart/up/down execution, or Docker prune execution still fail. Optional `--out-json` and `--out-markdown` write deterministic local output files only when explicitly requested; otherwise output is stdout-only.
...
python scripts/windows_smoke_acceptance.py --memory-json windows-memory.json --json
...
`shellforgeai windows memory --json` is accepted as `--memory-json windows-memory.json`. The validator checks the `windows_memory` mode, Windows platform, read-only/no-mutation flags, bounded no-PowerShell/no-WinRM/no-model safety flags, a present `memory` object, sane integer byte fields when available, and numeric used percentage between 0 and 100. This artifact is local-only read-only evidence and does not perform memory cleanup, optimization, remediation, remote collection, or process/service control.
