...
| Reviewed-change artifact publication (PR317) | create exactly one new `<data_dir>/approved_change_artifacts/<bundle-id>/` directory holding the four PR316 files | a PR316-valid bundle, an exact `confirm_bundle_identity_sha256` match, a safe absolute existing `data_dir`; no CLI or natural-language entry point exists |
| Approval artifact publication (PR319) | create exactly one new `<data_dir>/approved_change_approvals/<approval-artifact-id>/` directory holding the one fixed `approved-change-approval.json` | an artifact built from one fully successful PR318 approval, a PR319-valid artifact, an exact `confirm_approval_artifact_identity_sha256` match, a safe absolute existing `data_dir`; no CLI or natural-language entry point exists |
...
## Governed Windows two-file runtime reconciliation boundary (PR313)

`windows.runtime_reconcile` is the second governed mutation lane and the first bounded local file mutation lane. It is reachable only through the standalone direct script `scripts/windows_runtime_reconcile_execute.py`, never from a product CLI command, `ask`, interactive mode, or natural language. It may reconcile exactly two fixed mappings — `config/profiles/inspect.yaml -> config/profiles/inspect.yaml` and `scripts/windows/sfai.cmd -> bin/sfai.cmd` — relative to an explicit staged source root and an explicit durable runtime root. There is no user-defined mapping, prefix match, glob, recursion, alias, `shellforgeai.cmd` reconciliation, package file, residue file, or backup cleanup.

Execution requires a local Windows host, one saved PR305 packet that passes the maintained acceptance validator with status `ready` or `no_change`, one or two freshly generated PR304 artifacts that pass the maintained PR304 validator (stable-identity compared when two are supplied), explicit absolute roots matching the accepted plan, a ShellForgeAI-owned data directory, and `--confirm-plan-sha256` matching the accepted plan's canonical SHA-256 exactly. Natural-language confirmation, `--yes`, bare booleans, arbitrary mapping/destination/operation arguments, and wildcard paths are refused. The plan hash is recipe-specific authorization only: it is not a PR309 `ApprovalAttestation`, not portable approval, not authenticated enterprise identity, not approval of any other plan, and not approval of future drift. Missing or mismatched confirmation causes zero destination mutation, zero backup, zero temporary file, no receipt, and deterministic blocked output.

The saved plan is an authorization ceiling, not an instruction to overwrite. Immediately before any mutation the executor revalidates the packet and artifacts, re-evaluates both files through the maintained PR305 planner, and rehashes every source and destination. A planned create or replace narrows safely to a no-op when the destination already matches the validated source; a planned no-change never becomes a mutation. Source drift, a conflicting create destination, a disappeared or third-hash replace destination, a drifted or disappeared no-change destination, path escape, symlink/junction/reparse components, a missing destination parent, an oversized source, an invalid inspect profile, or a wrapper missing PR304 semantic markers all block the entire transaction without touching the other file.

Mutation begins only after every gate for both files has passed. Replacements take one verified same-directory backup created with exclusive creation, flushed, `fsync`ed, and hash-verified; creates and replacements are staged through verified same-directory temporary files and committed with `os.replace()` in exact allowlist order, followed by immediate hash verification. No direct destination write, append, in-place edit, shell copy, PowerShell, `cmd.exe`, `robocopy`, `xcopy`, or subprocess exists in this lane.

Compensation is bounded to files committed by the same confirmed execution: a committed replacement is restored only from that execution's verified backup, and a committed create is removed only when it did not exist before, still matches the expected source hash, and is the exact allowlisted destination. A drifted created file is never deleted and incomplete recovery is reported honestly. There is no post-success rollback command, no automatic restoration after a post-verification failure, and no backup pruning or retention lane — backups are retained.

Every confirmed execution that reaches current-state evaluation writes a ShellForgeAI-owned non-overwriting receipt bundle under `<data-dir>/windows_runtime_reconcile_receipts/`. The safety ledger keeps `cleanup_executed`, `remediation_executed`, `rollback_executed`, `recovery_executed`, `software_install_executed`, `software_uninstall_executed`, `service_control_executed`, `process_termination_executed`, `registry_modified`, `execution_policy_modified`, `powershell_executed`, `winrm_used`, `qga_used`, `remote_execution`, `subprocess_executed`, `shell_executed`, `shell_true`, `arbitrary_command_execution`, `natural_language_execution`, `docker_compose_executed`, `container_restarted`, `network_call`, `model_called`, `secret_read`, and `auth_cache_read` false, and `exact_two_file_allowlist` true. Blocked and unsupported results report `read_only=true` and `mutation_performed=false`; successful mutation reports `read_only=false` and `mutation_performed=true`. No safety flag may overstate success.

Receipts and logs record only fixed relative paths, hashes, counts, stable classifications, root fingerprints, bounded sanitized error summaries, and backup paths relative to the durable runtime root. File contents, wrapper text, YAML contents, environment dumps, process environment, credentials, secrets, tokens, auth-cache data, and sensitive absolute staged/durable paths are never recorded, and the receipt validator rejects any receipt that carries them. Post-change verification through `scripts/windows_runtime_reconcile_verify.py` is read-only and performs no repair or rollback; the PR312 V1 quick and standard checks from real `C:\Windows\System32` remain separate operator-run acceptance.

Existing safety boundaries are unchanged: the Docker disposable-restart lane, receipt recovery lane, cleanup refusal, remediation refusal, and natural-language mutation refusal all keep their current gates.

### Exact destination-parent contract (PR313, contract version 1)

`windows.runtime_reconcile` may create directories, but only the exact `config` and `config/profiles` components beneath an already-existing durable runtime root, and only to satisfy the fixed `config/profiles/inspect.yaml` destination. The wrapper parent `bin` is never created: a missing or unsafe `bin` remains a blocking runtime-layout failure. The durable runtime root must already exist as an absolute, safe, non-reparse directory matching the accepted plan; PR313 never creates it and never discovers it through PATH, home search, drive enumeration, globbing, or parent walking. No generic `mkdir`, installer, bootstrapper, directory-repair, runtime-layout command, or caller-supplied directory exists.

Parent metadata lives inside the PR305 packet, so it is inside the canonical JSON and therefore inside `--confirm-plan-sha256`. Any change to parent state, creation permission, creation chain, blocker, relative path, or contract version changes the required confirmation. Plans generated before this contract carry no `destination_parent_contract_version` and are refused outright rather than silently upgraded.

The saved parent state is an authorization ceiling revalidated immediately before preparation: a saved `present` parent must still be a safe directory; a saved creation chain may narrow to the still-missing exact suffix or to no action at all, but never grow or move. Any parent blocker for either file blocks the whole transaction before any directory creation, backup, temporary file, atomic replacement, or file mutation.

Directories are created one exact component at a time with `os.mkdir` — never `parents=True`, `os.makedirs` over caller-supplied paths, a shell, PowerShell, `cmd.exe`, a subprocess, or any recursive operation — with safe-prefix, absence, containment, and reparse rechecks before each component and existence, directory-ness, and non-reparse verification after. On failure, file compensation runs first and this execution's own directories are removed in reverse order, only when authorized, still directories, non-reparse, contained, and empty. Pre-existing and non-empty directories are never removed, nothing is removed recursively, and incomplete compensation is reported honestly. Successful executions retain the created directories as intended durable runtime state.

The safety ledger adds `parent_directory_create_executed` and `parent_directory_compensation_executed`. `cleanup_executed`, `rollback_executed`, and `recovery_executed` remain false: removing this execution's own empty directories is bounded transaction compensation, not a cleanup, rollback, or recovery lane. Receipts record only fixed relative directory information and never an absolute parent path, and the read-only verifier confirms retained directories remain safe without creating, repairing, or removing anything.

## PR317 governed reviewed-change artifact publication boundary

PR317 adds one narrow explicit-confirmed `ARTIFACT_WRITE` surface and nothing else. It publishes an already-valid PR316 reviewed-change artifact bundle beneath exactly one fixed ShellForgeAI-owned subtree, `<data_dir>/approved_change_artifacts/<bundle-id>/`, and offers one read-only loader for it. It is documented in full in [Approved Change Artifact Persistence](APPROVED_CHANGE_ARTIFACT_PERSISTENCE.md).

The surface is limited by construction. It accepts no arbitrary output path, caller-defined publication root, caller-defined final directory name, or caller-defined filename; the publication root name and all four filenames are fixed literals. It makes no host configuration, service, registry, package, Docker, Compose, PowerShell, WinRM, QGA, model, provider, network, shell, or subprocess call, and there is no CLI command, interactive route, or natural-language path that reaches it. Nothing outside `<data_dir>` can be written, and within `<data_dir>` nothing outside the fixed publication root can be written.

Publication requires two independent gates: the bundle must pass fresh maintained PR316 validation, and `confirm_bundle_identity_sha256` must equal that exact bundle identity under `hmac.compare_digest`. The prefixed bundle ID is never accepted as the confirmation value and confirmation is never inferred from the bundle. An invalid bundle or a mismatched confirmation performs zero filesystem access, zero directory creation, zero file creation, zero cleanup, and zero persistence.

There is no overwrite path. All four files are prepared and verified inside a private temporary sibling directory, the whole prepared bundle is reconstructed and revalidated, and one atomic no-replace directory transition publishes it — Linux `renameat2(..., RENAME_NOREPLACE)`, Windows `MoveFileExW` without `MOVEFILE_REPLACE_EXISTING`. No replace-capable primitive is reachable: `os.replace`, `os.rename`, `shutil`, and recursive deletion do not appear in the module. When a platform offers no proven atomic no-replace directory primitive, publication fails closed instead of downgrading. An existing fully valid byte-identical bundle returns `bundle_already_present` with nothing written; any conflicting, invalid, unsafe, or ambiguous existing directory blocks and is never repaired, replaced, quarantined, renamed, deleted, merged, or written around, including when it appears in a race between preparation and commit.

Deletion is bounded to the invocation's own unpublished temporary directory: only the exact files it created are removed, unknown entries are preserved, the directory is removed only when empty, and the publication root is removed only when this invocation created it and it is still empty. A pre-existing publication root is never removed and a published bundle is never removed, not even after a post-publication verification failure — that case is reported as `published_verification_failed` and retained for operator investigation.

The safety ledger is truthful rather than flattering. A successful first publication reports `read_only=false`, `mutation_performed=true`, `artifact_write_performed=true`, `publication_performed=true`, and `persistence_performed=true`; an existing identical bundle reports read-only behaviour with no persistence claim; a pre-commit failure after temporary writes still reports mutation, because a temporary write followed by cleanup is a filesystem mutation. `overwrite_performed` and `host_configuration_mutation_performed` are always false, and `approval_created`, `contract_created`, `receipt_created`, `capability_support_evaluated`, `capability_supported`, `approval_evaluated`, `authorization_evaluated`, `execution_allowed`, and `execution_available` are always false with `execution_status=not_executed`.

Persistence is not approval, not authorization, not an `ApprovedChangeContract`, not capability support, not receipt linkage, and not execution confirmation or eligibility. A published bundle is unapproved, unbound, and non-executable. Reviewed artifacts may carry operational context and must be reviewed before sharing; no redaction is performed because redaction would change reviewed identity.
...
## PR318 explicit approval binding for persisted reviewed changes

PR318 adds one read-only operation and no mutation surface at all. It is documented in full in [Approved Change Approval Workflow](APPROVED_CHANGE_APPROVAL_WORKFLOW.md).

**Explicit approval metadata is not authenticated identity.** `approved_by`, `approved_at`, and `reason` are explicit caller inputs and remain self-asserted. PR318 authenticates nobody, validates no role, integrates no identity provider, and never infers the actor from the OS user, environment, GitHub account, hostname, token, certificate, credential wrapper, or PR314 reviewer provenance. `reviewed_by` is never transformed into `approved_by`, and reviewer provenance is never approval.

**Approval binding is not authorization.** A verified `ApprovedChangeContract` states only that a named actor asserted approval of one exact PR309 subject SHA-256 at one exact stated time, under the scope `exact_subject_only`. The explicit bundle-identity confirmation selects which persisted reviewed source bundle is read; it never widens that scope. Two bundles sharing one subject identity remain separately confirmed, and one bundle's confirmation can never authorize loading another.

**A valid approved contract grants no execution eligibility.** PR318 consults no capability registry, never calls `validate_approved_change_contract`, evaluates and binds no capability, runs no current-state preflight, performs no subject-to-live-plan comparison, creates and links no receipt, and integrates no PR313 lane. Every result reports `capability_support_evaluated=false`, `capability_supported=false`, `preflight_evaluated=false`, `receipt_created=false`, `receipt_linked=false`, `execution_allowed=false`, `execution_available=false`, and `execution_status=not_executed`.

**The operation is read-only with respect to filesystem and host state.** It reaches persisted data only through the maintained PR317 exact-ID loader, accepts no arbitrary path, and creates no file, directory, or temporary object: `open`, `os`, `pathlib` at runtime, `shutil`, `tempfile`, subprocess, shell, sockets, Docker/Compose, PowerShell/WinRM/QGA, and model or provider calls are all absent from the module. Persisted bytes are never rewritten, repaired, or republished, and a successful approval binding leaves the persisted tree byte- and mtime-identical. The approval, the contract, and the result itself exist in memory only — nothing is persisted, so `artifact_write_performed`, `publication_performed`, `persistence_performed`, `approval_persisted`, `contract_persisted`, `mutation_performed`, and `host_configuration_mutation_performed` are always false, with `read_only=true`. Creating immutable in-memory approval metadata is not a host or filesystem mutation.

**No natural-language approval route exists.** PR318 adds no CLI command, no interactive route, no natural-language or implicit approval, and no model-generated approval. The decision must be exactly the literal `approve`; aliases, case and whitespace variants, truthy values, and omitted or defaulted decisions are all refused, and an invalid decision reaches no filesystem primitive. No legacy `Proposal` is converted, read, moved, or marked approved, and no PR309 approval is bridged into the legacy approval queue. PR318 adds no rejection, revocation, cancellation, expiration, or approval-state transition; approval persistence itself remains deferred.

## PR319 governed approval-artifact persistence boundary

PR319 adds one narrow explicit-confirmed `ARTIFACT_WRITE` surface and nothing else. It durably records an approval PR318 already created, beneath exactly one fixed ShellForgeAI-owned subtree, `<data_dir>/approved_change_approvals/<approval-artifact-id>/`, holding exactly one fixed `approved-change-approval.json`, and offers one exact-ID read-only loader for it. It is documented in full in [Approved Change Approval Artifact Persistence](APPROVED_CHANGE_APPROVAL_ARTIFACT_PERSISTENCE.md).

**Durable approval remains self-asserted.** The persisted `approved_by`, `approved_at`, and `reason` are exactly PR318's explicit caller inputs. PR319 authenticates nobody, validates no role, integrates no identity provider, and never infers or overwrites the actor from the OS user, environment, GitHub account, hostname, token, certificate, credential wrapper, or PR314 reviewer provenance. Writing an approval to disk does not make it authenticated.

**Persistence is not authorization.** A persisted `ApprovedChangeContract` states only that a named actor asserted approval of one exact PR309 subject SHA-256 at one exact stated time, under the scope `exact_subject_only`, bound to one exact PR316 source bundle. PR319 consults no capability registry, never calls `validate_approved_change_contract`, evaluates and binds no capability, runs no current-state preflight, creates and links no receipt, and integrates no PR313 lane. Every result reports `capability_support_evaluated=false`, `capability_supported=false`, `preflight_evaluated=false`, `receipt_created=false`, `receipt_linked=false`, `execution_allowed=false`, `execution_available=false`, and `execution_status=not_executed`. A persisted approval artifact grants no execution eligibility.

**An immutable approval artifact is not mutable approval state.** The artifact records one approval *event*. There is no status field, no revocation, no cancellation, no expiration, no supersession, no quorum or multi-approver semantics, no approval queue, and no state transition of any kind. There is also no `latest`, `current`, or "most recent" resolution and no discovery beyond one exact `aca_` artifact ID.

**No overwrite, repair, or deletion exists.** Publication requires two independent gates: the artifact must pass fresh maintained PR319 validation, and `confirm_approval_artifact_identity_sha256` must equal that exact recomputed artifact identity under `hmac.compare_digest`. The prefixed `aca_` ID, the source bundle identity, and the subject SHA-256 are never accepted in its place, and a malformed or mismatched confirmation performs zero filesystem access. The fixed file is prepared and verified inside a private temporary sibling, the artifact is reconstructed and revalidated from the prepared bytes, and one atomic no-replace directory transition publishes it — Linux `renameat2(..., RENAME_NOREPLACE)`, Windows `MoveFileExW` without `MOVEFILE_REPLACE_EXISTING`. No replace-capable primitive is reachable, and an unsupported platform fails closed rather than downgrading. An existing fully valid byte-identical artifact returns `approval_artifact_already_present` with nothing written and no timestamp refreshed; any conflicting, invalid, unsafe, or ambiguous existing directory blocks and is never repaired, replaced, quarantined, renamed, deleted, merged, or written around, including when it appears in a race between preparation and commit. Deletion is bounded to the invocation's own unpublished temporary directory, and a published approval artifact is never removed — not even after a post-publication verification failure, which is reported as `published_verification_failed` and retained for operator investigation. Retention and cleanup for these artifacts remain unsupported and deferred.

**The PR317 reviewed-change subtree stays read-only.** Source bundles are reached only through the maintained PR317 exact-ID loader. PR319 never calls PR317's publisher, never accepts an arbitrary source path, and leaves every persisted bundle byte and mtime unchanged; `source_bundle_mutation_performed` is always false. Nothing outside `<data_dir>` can be written, and within `<data_dir>` nothing outside the fixed `approved_change_approvals` root can be written.

**No natural-language approval or persistence route exists.** PR319 adds no CLI command, no interactive route, no natural-language or implicit approval, and no model-generated approval or persistence. No legacy `Proposal` is converted, read, moved, or marked approved, no legacy approval queue directory is read or created, and no PR309 approval artifact is bridged into the legacy queue. PR319 creates no new approval and no new contract: `approval_created` and `contract_created` are always false.

Existing safety boundaries are unchanged: the Docker disposable-restart lane, the receipt recovery lane, cleanup refusal, remediation refusal, the PR313 Windows reconciliation lane, and natural-language mutation refusal all keep their current gates.

## PR320 bounded read-only approval-artifact inventory

PR320 adds **no mutation surface at all**. It adds exactly one bounded, deterministic, read-only inventory over the fixed PR319 approval-artifact root and nothing else. It is documented in full in [Approved Change Approval Inventory](APPROVED_CHANGE_APPROVAL_INVENTORY.md).

**Inventory is read-only.** The operation takes only an explicit `data_dir`, derives the fixed `<data_dir>/approved_change_approvals` root, and enumerates its direct children once. It opens no file for writing, creates no file or directory — including the fixed root itself when it is absent — renames, replaces, unlinks, removes, or truncates nothing, uses no temporary directory, and refreshes no timestamp. Every result reports `read_only=true`, `mutation_performed=false`, `artifact_write_performed=false`, `publication_performed=false`, `persistence_performed=false`, and `host_configuration_mutation_performed=false`.

**Inventory is not authorization and is not capability support.** A returned summary states only that one persisted approval artifact validated through the maintained PR319 exact-ID loader. PR320 consults no capability registry, never calls `validate_approved_change_contract`, evaluates and binds no capability, runs no current-state preflight, creates and links no receipt, and integrates no PR313 lane. Every result reports `authorization_evaluated=false`, `capability_support_evaluated=false`, `capability_supported=false`, `capability_bound=false`, `preflight_evaluated=false`, `receipt_created=false`, `receipt_linked=false`, `execution_allowed=false`, `execution_available=false`, and `execution_status=not_executed`. Persisted `approved_by` remains self-asserted metadata, not authenticated identity, and reviewer provenance is still not approval.

**Inventory selects no approval.** Entries are ordered lexicographically by exact `aca_` artifact ID and by nothing else — never by filesystem mtime, approval time, actor, source bundle, or subject. There is no `latest`, `current`, or "most recent" resolution, no preferred-artifact resolution, no filtering, search, ranking, or pagination, and no selected-artifact field; every result reports `approval_selected=false`. An exact `aca_` artifact ID remains required to load the complete artifact and its approval reason.

**An anomalous inventory is explicitly incomplete.** Only a fully clean scan and an empty root report `inventory_complete=true`. Every unexpected name, symlink or reparse point, non-directory, uninspectable entry, entry that disappears or changes identity mid-scan, and artifact the PR319 loader rejects becomes an explicit anomaly with `inventory_complete=false`. Nothing is ever silently skipped, and no automated consumer may treat an incomplete inventory as a complete approval set. Root-safety failures, a mid-scan root-identity change, and exceeding the fixed 1024-entry bound all fail closed and return no entries at all rather than a partial list.

**No write, repair, cleanup, or deletion occurs.** An unexpected or invalid direct child is reported and left exactly as it is: it is never followed, recursed into, opened, repaired, overwritten, renamed, quarantined, or deleted. No persisted index, pointer, or cache is created or consulted, so discovery adds no new persisted state and no new subtree. The fixed entry bound protects a read-only operation from unbounded directory and validation work; it is not a retention policy.
...
The separate PR304 freshness authority classifies only whether the confirmed
pair's earliest capture start is at most five minutes old according to one
untrusted evaluator-local UTC read. It neither trusts or synchronizes clocks
nor revalidates current state. Fresh is not permission to act, approval
freshness, authorization, preflight, or execution eligibility; packet status
remains independent and TOCTOU remains unresolved.
