...
PR99 adds `shellforgeai remediation self-test` as a read-only remediation-lane readiness doctor. By default it never executes remediation/rollback/cleanup, never restarts containers, never runs Docker Compose mutation, and never enables natural-language execution. If an optional live lane is added in a future change, it must remain explicitly gated by dedicated flags and disposable-target checks.
