...
Persisted approval **discovery beyond exact ID** is deliberately absent from PR319 itself: this module offers no listing, search, index, or "most recent approval" resolution. PR320 adds one bounded read-only inventory over the fixed `approved_change_approvals` root — documented in [Approved Change Approval Inventory](APPROVED_CHANGE_APPROVAL_INVENTORY.md) — which enumerates direct children only, validates exact `aca_` candidates solely by calling the loader above, and still resolves no `latest`, `current`, or "most recent" approval and creates no index or pointer. An exact `aca_` artifact ID remains the only way to retrieve a complete artifact.

## Next dependency

Stage B remains incomplete. The next focused dependency remains explicitly **non-execution** work.

Persisted approval discovery beyond exact ID landed in PR320 as one bounded read-only inventory that selects nothing. Typed capability-support declaration and evaluation landed in PR321 in [Approved Change Capability Support](APPROVED_CHANGE_CAPABILITY_SUPPORT.md): one immutable maintained catalog with a deterministic identity, declaring exactly `windows.runtime_reconcile`, plus one read-only evaluator that reaches persisted approvals only through this exact-ID loader and decides support only through the maintained PR309 `validate_approved_change_contract`. PR321 selects no approval, writes nothing, and binds no capability, so this module's overwrite, revocation, and mutable-state prohibitions are unchanged.

The exact read-only in-memory capability binding landed in PR322 in [Approved Change Capability Binding](APPROVED_CHANGE_CAPABILITY_BINDING.md): one immutable source-maintained lane declaration for the named PR313 governed Windows lane plus one operation that confirms support through PR321 and obtains the exact artifact identity and subject SHA-256 through this exact-ID loader alone. PR322 is the third and last permitted read-only consumer of this loader. It never calls the publisher or the builder, persists no binding artifact, adds no new subtree or file, and selects no approval, so this module's overwrite, revocation, and mutable-state prohibitions are unchanged there too.

Everything else stays deferred to PR322 or later: approval revocation, cancellation, expiration, supersession, quorum or multi-approver semantics, authenticated identity, role validation, identity-provider integration, exact capability binding, binding `windows.runtime_reconcile` to the PR313 lane, current-state execution preflight, subject/live-plan comparison, receipt creation and linkage, execution eligibility, Stage C execution, CLI approval commands, natural-language approval or persistence, persisted-artifact deletion, and retention and cleanup policies.
