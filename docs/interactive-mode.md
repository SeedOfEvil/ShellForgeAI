...
### Windows crash-health questions

On a native Windows host, bounded interactive questions such as `Is anything crashing?`, `Are any processes crashing?`, and `Do you see any crash signals?` use the existing Windows evidence-backed analytical path. This is an interactive-only route correction: explicit Docker, container, or Compose targets retain their Docker route, and unqualified Linux-host questions retain their existing Linux/generic behavior. Explicit Windows questions from a non-Windows host remain guidance-only and do not probe Windows.

The initial turn collects the bounded Windows packet once, renders and flushes its deterministic process, service, memory, and volume evidence before synchronous model synthesis, and presents accepted provider output only as a supplemental assessment. Point-in-time process or service state is not proof of historical stability. Missing Event Log, crash-record, dump, or service-recovery history is reported as a limitation, so the answer cannot conclude that nothing has crashed. Provider failure or rejected Windows model output leaves the deterministic evidence intact and does not retry. Bounded analytical follow-ups reuse the retained deterministic facts and limitations without recollection, while keeping any accepted selected signal model-derived. No command execution, token streaming, background work, new collector, or mutation is introduced.

### Windows evidence-context routing

Generic running-system inventory questions on a Windows host (for example `What is running on this system?` or `Which running items deserve attention and why?`) use a shared bounded Windows intent before generic Linux diagnosis. The `ask` label is `Windows running-system inventory`. Interactive mode and `ask` each build the existing read-only Windows evidence packet exactly once — host/platform basics, memory posture, disk/root usage, bounded native process and service previews, and their collection limits — and pass it into the evidence-first model context. Explicit Docker, Compose, container-only, Linux, and systemd scope is not captured. Container visibility is reported as unavailable because the Windows packet does not collect it; process or service names are never used to infer containers. This analytical route adds no natural-language mutation authority.

Model output is captured before stdout and gated: project/policy preamble (AGENTS.md, invariants, "Understood. I'll operate within..."), provider/model/usage metadata as the primary answer, and Docker/container-first framing are rejected and replaced with a deterministic evidence-grounded Windows answer (raw rejected output is kept only in the `model-response.md` audit artifact). When the packet is thin, the answer says what is visible, states that process/service detail is not in the current evidence packet, and points to `sfai.cmd windows processes --json --limit 10`, `sfai.cmd windows services --json`, and `sfai.cmd windows status --json` instead of inventing facts. The packet is assembled read-only from existing payload builders: no shell, no PowerShell, no WinRM/remoting, no subprocess execution, and no mutation.

Model synthesis projects that unchanged raw packet into a Windows-native prompt using Windows service state, Event Log, drive/volume, process, and memory vocabulary. Precise Linux-primary operational instructions are captured and rejected rather than shown; the maintained deterministic fallback remains read-only and evidence-grounded. Deterministic Windows status, next-check, service, and mutation-refusal routes remain model-free.
...
facts, limitations, safe read-only next checks, and at most one separately marked
model-derived selected signal.

Narrow analytical follow-ups asking for the strongest CPU, memory, disk/volume, or
process signal, and asking for the best next check based on evidence already
collected, reuse that packet without recollecting the same snapshot. Retained
deterministic evidence is rendered and flushed first through the evidence-first
presentation; synchronous model assessment remains supplemental. A newer collection
replaces the packet, while a session, platform, target, or collection mismatch
invalidates reuse. Provider failure or Windows response rejection leaves the
deterministic evidence intact and does not retain the failed or rejected response.

This handoff adds no token streaming, background work, autonomous execution, command
execution, or mutation. Natural-language mutation refusal remains authoritative.
