...
Generic running-system inventory questions on a Windows host (for example `What is running on this system?`) use a shared bounded Windows intent before generic Linux diagnosis. Interactive mode and `ask` each build the existing read-only Windows evidence packet exactly once — host/platform basics, memory posture, disk/root usage, bounded native process and service previews, and their collection limits — and pass it into the evidence-first model context. Explicit Docker, Compose, container-only, Linux, and systemd scope is not captured. Container visibility is reported as unavailable because the Windows packet does not collect it; process or service names are never used to infer containers.
