- Current CLI command-module split: `src/shellforgeai/cli.py` remains root Typer wiring while read-only/status/reporting handlers live under `src/shellforgeai/commands/`, including governed receipt audit/reporting (`recipes receipt audit`, `audit-bundle`, `audit-bundle-validate`, `integrity`, and `explain`) in `commands/receipt_audit.py`. These receipt audit/reporting surfaces are unchanged: audit, integrity, explain, and bundle validation remain read-only; audit-bundle remains bounded ShellForgeAI-owned artifact export only; governed execution and recovery execution stay outside the module. Future CLI refactors should continue running the PR184 command-surface golden guardrail.
