...
The evidence pair carries bounded collector-owned local-UTC and monotonic capture chronology, and the separate fixed-policy PR304 freshness evaluator is implemented. The sequence is: exact PR304 evidence-set identity/binding → bounded capture chronology → PR304 evidence freshness evaluation → future authenticated identity → future authorization → future execution preflight. Approval freshness, authenticated identity, authorization, preflight, receipts, and execution remain incomplete.
