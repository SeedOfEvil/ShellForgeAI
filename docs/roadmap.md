...
The evidence pair carries bounded collector-owned local-UTC and monotonic capture chronology, and the separate fixed-policy PR304 freshness evaluator is implemented. The sequence is: exact PR304 evidence-set identity/binding → bounded capture chronology → PR304 evidence freshness evaluation → Windows current-process token identity evidence → future exact identity-to-approval/provenance binding → future approval freshness → future authorization → future execution preflight. Identity binding, approval freshness, authorization, preflight, receipts, and execution remain incomplete. Windows token evidence is OS process identity only, not physical-human identity or permission to act.
