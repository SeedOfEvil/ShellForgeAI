- Current: ShellForgeAI includes a narrow read-only platform detection and Windows platform-doctor evidence foundation for Windows/PowerShell V1 preparation. `shellforgeai platform doctor` and `shellforgeai platform doctor --json` classify Linux, Windows, Darwin, and unknown platforms with Python standard library metadata only. Linux remains the supported Linux/Docker operational lane; Windows now reports the limited `windows_read_only_doctor_v1` lane with deterministic evidence for OS family/name, Windows version/build when available through Python, architecture, Python version/platform, and safe PowerShell/pwsh path availability signals. Darwin and unknown remain unsupported for current operational lanes. A reusable structured unsupported-platform payload records requested lane, detected platform, reason, safe next command, `read_only=true`, and `mutation_performed=false`. This is still evidence-only: no PowerShell execution, WinRM/PSRemoting, Windows services/processes/event logs, Windows broad host probing, Docker/Compose mutation, installs, packaging build, network/model call, secret/auth-cache read, remediation, rollback, recovery, cleanup, source-action execution, production restart, natural-language execution, or `shell=True` is added. Windows Server 2025 testing should validate this platform-doctor layer before any broader Windows evidence gathering. Future sequence: broader Windows read-only doctor/status prototype, then packaging/install spike.
