...
- PR330 preserves Windows service-health route correctness and the bounded
  ...
  session-local analytical evidence-continuity handoff.
...
- PR333 reuses only the latest bounded evidence packet for a narrow class of
  ...
  introduced.
- PR334 is the narrow Windows interactive crash-health routing-correctness
...
The Stage B chain now has a read-only point-in-time sequence: persisted approval → capability binding → canonical plan link → persisted `acpl_` link artifact → exact PR338 live current-state revalidation → canonical current-state evidence-content identity → future separately reviewed authorization → future execution preflight. The evidence identity adds no clock or timestamp, does not establish freshness, and may be identical across separate observations with identical facts. Authenticated human identity, authorization, execution preflight, receipt linkage, and execution remain deferred and incomplete.
...
The evidence pair carries bounded collector-owned local-UTC and monotonic capture chronology, and the separate fixed-policy PR304 freshness evaluator is implemented. The sequence is: exact PR304 evidence-set identity/binding → bounded capture chronology → PR304 evidence freshness evaluation → Windows current-process token identity evidence → exact in-memory identity-evidence-to-approval-provenance binding → exact approval-assertion chronology/age evidence → fixed approval-assertion recency policy → future authorization → future execution preflight. The provenance binding compares neither SID nor LUID with self-asserted `approved_by`; it authenticates no human, persists nothing, and grants no authority. The approval-assertion age authority measures the persisted self-asserted, untrusted `approved_at` value only against untrusted evaluator-local UTC; the fixed 24-hour policy then classifies only whether that exact untrusted assertion chronology falls within its inclusive age window. This assertion-recency classification does not establish authenticated approval, approval validity, expiration, revocation, or authority. Authorization, preflight, receipts, and execution remain incomplete. Windows token evidence is OS process identity only, not physical-human identity or permission to act.

## Operator-parity trajectory

The versioned parity contract and deterministic audit harness establish the
measurement boundary. The focused sequence is: intent/safety classifier
separation; native evidence-router parity; evidence-first model orchestration;
shared refusal UX and scope hygiene; Windows UTF-8 evidence; latency
instrumentation/optimization; multi-turn evidence continuity; and final operator
parity certification. Each slice removes stable known-gap allowances only after
the shared semantic target is met.
