- Current: ShellForgeAI includes a narrow read-only platform detection foundation for Windows/PowerShell V1 preparation. `shellforgeai platform doctor` and `shellforgeai platform doctor --json` classify Linux, Windows, Darwin, and unknown platforms with Python standard library metadata only. Linux remains the supported Linux/Docker operational lane; Windows is recognized as `windows_v1_planned` with evidence collection unavailable; Darwin and unknown are unsupported for current operational lanes. A reusable structured unsupported-platform payload records requested lane, detected platform, reason, safe next command, `read_only=true`, and `mutation_performed=false`. This is platform detection and graceful unsupported output only: no PowerShell, WinRM/PSRemoting, Windows services/processes/event logs, Windows host probing, Docker/Compose mutation, installs, packaging build, network/model call, secret/auth-cache read, remediation, rollback, recovery, cleanup, source-action execution, production restart, natural-language execution, or `shell=True` is added. Windows Server 2025 testing starts at this platform-doctor/unsupported-output layer before any Windows evidence gathering. Future sequence: platform detector, Windows read-only doctor prototype, Windows read-only status/report prototype, then packaging/install spike.
