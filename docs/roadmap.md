...
- PR149 (June 2, 2026): V2 `verify` current-state verification milestone. The golden path now reads `status -> triage -> propose -> apply-preview -> verify`; the new command reports ok/degraded/blocked/unknown from current deterministic status/triage evidence and supports brief/JSON/target plus `--from-status`/`--from-triage`/`--from-propose`/`--from-apply-preview` forms. Verify never assumes an apply/remediation happened and creates no receipt: with `--from-apply-preview`/`--from-propose` it states that no apply receipt was provided and verifies current observed state only. Deterministic ask routing covers verify phrasing (`verify status`, `did anything improve?`, `is it fixed?`) and refuses mutation phrasing (`verify and restart`, `apply and verify`); interactive allowlist/help cover the verify forms. It is read-only only: no apply, mission, plan artifact, remediation receipt, cleanup/remediation/rollback execution, Docker/Compose mutation, container restart, production restart, `shell=True`, arbitrary command execution, natural-language mutation, or model call was added.
...
- Completed: PR149 V2 read-only `verify` entrypoint for current-state verification. `verify` assumes no apply/remediation happened, creates no receipt, and proves no completed remediation without a future receipt/artifact.
