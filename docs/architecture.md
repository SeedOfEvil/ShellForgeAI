...
| `core/approved_change_persisted_plan_link_current_state_evidence.py` | The one canonical PR338 current-state evidence consumer: accepts only PR338's explicit upstream inputs, invokes the maintained PR338 authority exactly once, and constructs immutable bounded evidence plus a deterministic non-circular SHA-256 only from an internally coherent `current_state_confirmed` result. It verifies the exact identity chain and fixed ordered two-mapping facts without inspecting governed roots itself; includes only relative paths and PR338 root fingerprints; reads no clock; adds no timestamp, freshness, authentication, authorization, readiness, preflight, receipt, persistence, or execution authority; and accepts no caller-supplied revalidation result. |
