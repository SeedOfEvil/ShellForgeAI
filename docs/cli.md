...
| `approvals create <session>` / `list` / `show <id>` / `approve <id> --reason ...` / `reject <id> --reason ...` / `cancel <id> [--reason ...]` / `validate <id>` | Manage mutation proposal objects (read-only metadata). Proposals are derived from `runbook.json` and live under `<data_dir>/approvals/{pending,approved,rejected,canceled}/<id>.proposal.json`. Approval is a paper trail — it does **not** execute anything. |
