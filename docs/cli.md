...
| `model doctor [--json] [--live-probe] [--receipt-out DIR]` | Provider doctor. Default mode is read-only/no-call/no-network and reports `auth_readiness=not_verified` when live auth was not requested. `--live-probe` explicitly performs one bounded fixed-payload auth/readiness probe through the configured model client, executes no tools, accepts no freeform prompt, and performs no mutation. `--receipt-out DIR` writes bounded JSON/Markdown/manifest/checksum receipt files. |
...
An explicit cross-platform canonical rendering mode is also available:
...
### Windows interactive read-only phrase routing

Inside `shellforgeai interactive`, explicit Windows read-only phrases such as `show me the windows status`, `windows doctor`, `windows evidence`, and `windows processes limit 10` route deterministically to safe command guidance for the Windows wrapper (`sfai.cmd windows status --json`, `sfai.cmd windows doctor --json`, `sfai.cmd windows evidence --json`, and `sfai.cmd windows processes --json --limit 10`). `/pending` records the active `windows-local-read-only` context for these requests. This is guidance only: no shell, PowerShell, WinRM/PSRemoting, subprocess, Docker/Compose mutation, cleanup, remediation, rollback, recovery, service/process/disk mutation, model fallback, or broad natural-language execution is added.
...
On a native Windows host, generic questions asking what is running or requesting an evidence-backed process/service inventory select the existing bounded Windows evidence packet rather than Linux process or systemd diagnosis. Native process and service counts, preview limits, and truncation remain available to model synthesis after the evidence stage. Windows container inventory is not collected and is stated as unavailable rather than inferred. Explicit Docker/Compose or Linux/systemd scope retains its existing route, while service-only, performance, network, status, mutation-refusal, and shell-refusal routes keep higher precedence.
