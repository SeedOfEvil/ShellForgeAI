...
| `session summary history [--limit N] [--json]` / `session summary compare <before> <after> [--only-changed] [--include-stable] [--json]` / `session summary compare-latest [--only-changed] [--json]` | Read-only history and compare workflow for saved interactive summary artifacts. History lists saved summaries and latest handoff metadata. Compare validates and reads two saved summaries, then reports changed checks, findings, refusals, safe next commands, artifact references, runtime visibility, and safety flags. `compare-latest` compares the newest two saved summaries or returns controlled `empty` / `not_enough_data` status. These commands never rerun collectors, call the model, execute shell, or mutate Docker/Compose/system state. |
