...
PR321 supplies the one maintained answer to "which capabilities are supported?" in [Approved Change Capability Support](APPROVED_CHANGE_CAPABILITY_SUPPORT.md). It defines one immutable, source-maintained declaration catalog with a deterministic identity, declaring exactly `windows.runtime_reconcile`, and its evaluator passes exactly that catalog's capability IDs to this maintained validator. PR309 remains the validation authority and gains no registry, no default set, and no dynamic lookup of its own.
