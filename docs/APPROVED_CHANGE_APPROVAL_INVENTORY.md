...
Stage B remains incomplete. The next focused dependency remains explicitly **non-execution** work: a typed, read-only capability-support declaration and evaluation contract.

Everything else stays deferred to PR321 or later: typed capability-support declarations, a deterministic capability catalog identity, capability-support evaluation, exact capability binding, binding `windows.runtime_reconcile`, authenticated identity, role validation, identity-provider integration, current-state execution preflight, subject/live-plan comparison, receipt creation and linkage, execution eligibility, Stage C execution, approval revocation, cancellation, expiration, supersession, quorum or multi-approver semantics, filtered approval search, automatic artifact selection, `latest` or `current` resolution, persisted indices, CLI approval commands, natural-language approval, inventory, or persistence, artifact deletion, and retention and cleanup policies.
